  ...
  return (
    <div className="w-full flex items-center justify-center">
      <ProjectReviewCard
        ...
        troubles={[
          {
            problem: <>CI/CD 구축을 위해 HTTP 서버 배포를 계획했으나, <span className="inline xl:block xl:ml-12">프론트엔드 단독 프로젝트로 서버 관리가 불필요함.</span></>,
            solution: <>정적 빌드된 프론트엔드만 배포 가능한 Vercel 플랫폼으로 <span className="inline xl:block xl:ml-12">배포 환경 전환, GitHub Repository와 연동하여 자동 배포</span><span className="inline xl:block xl:ml-12">(CI/CD) 파이프라인을 간소화.</span></>,
            result: <>별도의 서버 구축, 관리 없이 배포 속도 단축 및 유지보수 <span className="inline xl:block xl:ml-12">효율성 향상.</span></>,
          },
          {
            problem: <>초기 교통 데이터는 표 형태로만 표시되어, 사용자가 시간대별 <span className="inline xl:block xl:ml-12">속도 변화를 직관적으로 파악하기 어려움.</span></>,
            solution: <>Chart.js를 도입하여 시간대별 평균 속도와 도로 유형별 <span className="inline xl:block xl:ml-12">평균 속도를 Line Chart를 통하여 데이터 시각화.</span></>,
