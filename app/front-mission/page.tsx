  ...
  return (
    <div className="w-full h-screen flex flex-row items-center justify-center pb-30">
      <ProjectReviewCard
        ...
        troubles={[
          {
            problem: <>localStorage에 토큰을 저장해 자동 로그인 선택 여부와 무관하게 <br /><span className="ml-12">항상 자동 로그인이 되는 현상 발생.</span></>,
            solution: <> “자동 로그인” 선택 시에만 localStorage, <br /><span className="ml-12">미선택 시 sessionStorage에 토큰 저장/복원.</span></>,
            result: <>자동 로그인 선택 시에만 브라우저 재시작 후 로그인 유지, <br /><span className="ml-12">미선택 시 브라우저 종료와 함께 로그아웃(세션 종료) 동작.</span></>,
          },
          {
            problem: <>로컬 환경에서는 정상 동작했으나, Vercel 배포 시 클라이언트 훅 <br /><span className="ml-12">사용으로 인해 정적 최적화 과정에서 하이드레이션 오류 발생.</span></>,
            solution: <>Suspense로 클라이언트 훅의 데이터 준비가 완료될 때까지 렌더링을 지연, <br /><span className="ml-12">force-dynamic 을 선언해 정적 최적화를 비활성화하여 SSR로 강제 전환.</span></>,
            result: <>Vercel 환경에서도 수정 페이지가 정상적으로 렌더링되며, <br /><span className="ml-12">초기 로딩 시점의 하이드레이션 오류와 UI 불일치가 해결됨.</span></>,
          },
        ]}
        drawbacks={
